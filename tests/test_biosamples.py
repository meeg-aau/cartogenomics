...
class TestBioSamples(unittest.TestCase):
    ...
    def test_get_biosample_with_ena_sample_accession(self):
        logging.info("\nTesting fetch biosample with ENA accession")
        biosamples_json = biosamples.get_basic_sample_data(self.ena_acc)
        assert len(biosamples_json) > 0
        assert biosamples_json["accession"] == "SAMN39868869"
        assert biosamples_json["name"] == "MFD04522"