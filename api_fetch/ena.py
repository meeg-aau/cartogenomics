...
class ENAClient:
    ...
    def get_all_run_accessions(self, acc: str) -> List[Dict[str, Any]]:
        """
        Return all runs associated with any ENA accession (run, experiment,
        biosample, sample).
        """
        ...
        for record in ena_data:
            samples = [
                ...
                record.get("secondary_sample_accession"),
            ]
            results.append(
                {
                    ...
                    "biosample": next(
                        (i for i in samples if i and i.startswith("SAM")), None
                    ),
                    "ena_sample": next(
                        (
                            i
                            for i in samples
                            if i and i.startswith(("ERS", "SRS", "DRS"))
                        ),
                        None,
                    ),
    ...
    def get_all_genome_accessions(self, acc: str) -> Dict[str, Any]:
        ...
        return {
            "genome_accession": (
                record.get("assembly_accession") if acc_type != "genome" else acc
            ),
