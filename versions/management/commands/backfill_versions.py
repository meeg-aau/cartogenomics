...
class Command(BaseCommand):
    help = "Backfill SampleVersion and GenomeVersion rows for existing records that have none."
    ...
    def _backfill_samples(self):
        already_versioned = set(
            SampleVersion.objects.filter(valid_to__isnull=True)
        ...
        for sample in Sample.objects.select_related("ingest").all():
            ...
            if sample.ingest_id is None:
                logger.warning(f"Sample id={sample.pk} ({sample}) has no ingest — skipping.")
                skipped += 1
                continue
    ...
    def _backfill_genomes(self):
        already_versioned = set(
            GenomeVersion.objects.filter(valid_to__isnull=True)
